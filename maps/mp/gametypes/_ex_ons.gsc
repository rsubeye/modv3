...
updateGametypeCvars()
{
	while(!level.ex_gameover && !game["matchpaused"])
	{
		...
		scorelimit = getCvarInt("scr_ons_scorelimit");
...
initFlags()
{
	...
	for(i = 0; i < flags.size; i++)
	{
		...
		hud_index = levelHudCreate("flag_" + flags[i].objective, undefined, 325 + 36 * i - 18 * (flags.size - 1), level.hud_ons_pos_y, 0.8, (1,1,1), 1, 1, "fullscreen", "fullscreen", "center", "middle", false, false);
...
flag()
{
	...
	for(;;)
	{
		...
		else if(isPlayer(player) && isAlive(player) && (player.pers["team"] != "spectator") && level.roundstarted && !level.roundended)
		{
			...
			{
				if(player canCapture(self))
					self startCaptureProgress(player, player.pers["team"]);
				else 
				{
					player iprintlnbold(&"MP_ONS_CANT_CAPTURE");
...
startCaptureProgress(player, team)
{
	...
	if(self.team != "none") 
	{
		...
		while(isDefined(self) && time < time_neutral)
		{
			...
			if(level.roundended || !isDefined(player) || !player canCapture(self))
			{
				levelHudSetAlpha("flag_" + self.objective, 0.8);
		...
		hud_index = levelHudCreate("flag_" + self.objective, undefined, 325 + 36 * self.objective - 18 * (level.flags.size - 1), level.hud_ons_pos_y, 0.8, (1,1,1), 1, 1, "fullscreen", "fullscreen", "center", "middle", false, false);
		if(hud_index != -1) levelHudSetShader(hud_index, level.hudicon_empty, 32, 32);
...
GetFlag(flag)
{
	...
	if(self.pers["team"] == "allies") 
	{
		...
		hud_index = levelHudCreate("flag_" + flag.objective, undefined, 325 + 36 * flag.objective - 18 * (level.flags.size - 1), level.hud_ons_pos_y, 0.8, (1,1,1), 1, 1, "fullscreen", "fullscreen", "center", "middle", false, false);
