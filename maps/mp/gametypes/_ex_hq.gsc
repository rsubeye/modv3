...
Callback_StartGameType()
{
	...
	if(!isDefined(game["precachedone"]))
	{
		...
		[[level.ex_PrecacheString]](&"MP_RESPAWN_WHEN_RADIO_NEUTRALIZED");
	}
	...
	if(level.respawndelay == 0) level.respawndelay = 10;
...
Callback_PlayerKilled(eInflictor, attacker, iDamage, sMeansOfDeath, sWeapon, vDir, sHitLoc, psOffsetTime, deathAnimDuration)
{
	...
	if((isDefined(self.pers["team"])) && (level.DefendingRadioTeam != self.pers["team"])) defendingBeforeDeath = false;

	defendingAfterDeath = false;
	if((isDefined(self.pers["team"])) && (level.DefendingRadioTeam == self.pers["team"])) defendingAfterDeath = true;

	allowInstantRespawn = false;
	if((!defendingBeforeDeath) && (defendingAfterDeath)) allowInstantRespawn = true;
...
respawn(updtimer)
{
	...
	while(isDefined(self.WaitingToSpawn) || ((self.pers["team"] == level.DefendingRadioTeam) && isDefined(self.WaitingOnNeutralize))) wait( [[level.ex_fpstime]](0.05) );
...
hq_radio_capture(radio, team)
{
	...
	{
		if(!isPlayer(players[i])) continue;
		players[i].WaitingToSpawn = undefined;
...
respawn_staydead(delay)
{
	...
	{
		playerHudSetKeepOnKill(hud_index, true);
		playerHudSetText(hud_index, &"MP_RESPAWN_WHEN_RADIO_NEUTRALIZED");
	}
...
updateTimer()
{
	...
	{
		if(self.pers["team"] == level.DefendingRadioTeam)
		{
