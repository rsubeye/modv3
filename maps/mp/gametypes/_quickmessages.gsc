init()
{
	...
	else
	{
		...
		if(level.ex_specials) [[level.ex_PrecacheMenu]](game["menu_quickspecials"]);
	}
...
quickrequests(response)
{
	...
	if(response == "1") thread extreme\_ex_firstaid::callformedic();
}
...
quickwarning(warning, range, warnown, warnenemy)
{
	...
	info = self extreme\_ex_utils::playersInRange(range);
