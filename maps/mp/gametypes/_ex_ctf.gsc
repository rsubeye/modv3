		...
		if(isPlayer(other) && isAlive(other) && (other.pers["team"] != "spectator"))
		{
			...
			if(other.pers["team"] == self.team) // Touched by team
			{
				if(self.atbase)
				{
					if(isDefined(other.flag)) // Captured flag
					{
						...
						other.pers["flagcap"]++;
						other thread [[level.pscoreproc]](level.ex_ctfpoints_playercf, "special");
				...
				else // Returned flag
				{
					...
					other.pers["flagret"]++;
					other thread [[level.pscoreproc]](level.ex_ctfpoints_playerrf, "special");
				}
			}
			else if(other.pers["team"] != self.team) // Touched by enemy
			{
				...
				if(self.atbase) // Stolen flag
				{
					...
					other thread [[level.pscoreproc]](level.ex_ctfpoints_playersf, "special");
				}
				else // Picked up flag
				{
					lpselfnum = other getEntityNumber();
					...
					if(!isDefined(other.flagDropped))
						other thread [[level.pscoreproc]](level.ex_ctfpoints_playertf, "special");
				}
