...
sprintMonitor()
{
	...
	while(1)
	{
		...
		if(isPlayer(self) && self useButtonPressed() && self.ex_sprinttime > 0 && !sprintstop)
		{
			...
			if(level.ex_sprintheavymg && isWeaponType(self.weapon["current"].name, "mobilemg"))
			{
			...
			// cannot sprint if carrying a rocket launcher (heavy panzer setting)
			if(level.ex_sprintheavypanzer && isWeaponType(self.weapon["current"].name, "rl"))
			{
			...
			if(!self.ex_sprinting && primary != game["sprint"])
			{
				...
				if(primary != self.weapon["oldprimary"].name || primaryb != self.weapon["oldprimaryb"].name)
				{
					...
					{
						// the knife perk could be in primaryb, so allow that to happen
