...
main(command)
{
	...
	switch(command)
	{
		...
		case "clanlogin_cmd_login":
		{
			...
			// pretend to come from server info menu to allow auto-assign bridging to occur
...
clanLoginTimeframe(seconds)
{
	...
	else self thread extreme\_ex_memory::setMemory("clan", "pin", level.ex_clanlogin_pin, level.ex_tune_delaywrite);
}
