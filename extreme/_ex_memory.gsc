...
init()
{
	...
	registerMemory("score", "points", 0, 0, level.MAX_SIGNED_INT, "int");
	registerMemory("score", "kills", 0, 0, level.MAX_SIGNED_INT, "int");
	registerMemory("score", "deaths", 0, 0, level.MAX_SIGNED_INT, "int");
	registerMemory("score", "bonus", 0, 0, level.MAX_SIGNED_INT, "int");
	registerMemory("score", "special", 0, 0, level.MAX_SIGNED_INT, "int");
	registerMemory("total", "points", 0, 0, level.MAX_SIGNED_INT, "int");
	registerMemory("total", "kills", 0, 0, level.MAX_SIGNED_INT, "int");
	registerMemory("total", "deaths", 0, 0, level.MAX_SIGNED_INT, "int");
	registerMemory("total", "bonus", 0, 0, level.MAX_SIGNED_INT, "int");
	registerMemory("total", "special", 0, 0, level.MAX_SIGNED_INT, "int");
	registerMemory("total", "time", 0, 0, level.MAX_SIGNED_INT, "int");
	//dumpMemoryStruct();
...
checkPlayerStruct()
{
	...
	if(!isDefined(self.pers["memory"]))
	{
		...
		// remember name, so we load from and save to same file, even if player changes name
		memName = sanitizeName(self.name);
		memGUID = self getGuid();
		switch(level.ex_memory_filename)
		{
			case 1:
				if(memGUID) memName = "" + memGUID;
				break;
			case 2:
				if(memGUID) memName = "" + memGUID + "-" + memName;
				break;
		}
		self.pers["memory"]["-name-"] = memName;
