...
prepSpawnpoints()
{
	...
	// show HQ radio array defined in level script. HQ entities are ignored in this case
	// custom HQ radios from maps\mp\gametypes\_mapsetup_chq_hq are processed in markSpawnpoints()
	if(isDefined(level.ex_spawnmarkers_type["hqr"]) && isDefined(level.radio))
	{
...
markSpawnpoints()
{
	// show custom radios from maps\mp\gametypes\_mapsetup_chq_hq, which are added in hq_setup
	// after running prepSpawnpoints()
	if(isDefined(level.ex_spawnmarkers_type["hqr"]) && isDefined(level.radio))
	{
