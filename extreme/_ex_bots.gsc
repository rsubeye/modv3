...
dbotStart()
{
	...
	logprint("Spawning " + level.ex_testclients + " test clients on request...\n");
	for(i = 0; i < level.ex_testclients; i++)
	{
		...
		for(j = 0; j < players.size; j++)
		{
			...
			{
				//logprint("Reusing existing bot " + player.name + "\n");
				bot = player;
				break;
			}
		}
		...
		else logprint("ERROR: failed to spawn bot " + i + "\n");
	}
...
dbotJoin(team)
{
	if(level.ex_classes == 1)
	{
		...
		// pass class system menu
		class = "class" + (randomInt(5) + 1);
		self notify("menuresponse", game["menu_classes"], class);
		wait( [[level.ex_fpstime]](0.5) );
	}
	else
	{
...
jumpGravity()
{
	...
	if(isDefined(self.next.next[0]))
	{
		...
		{
			wait( [[level.ex_fpstime]](0.5) );
			while(self.botorg.origin[2] > destOrg[2]) wait( [[level.ex_fpstime]](0.05) );
		}
		else
		{
			wait( [[level.ex_fpstime]](0.5) );
			while(self.botorg.origin[2] > destOrg[2]+32 ) wait( [[level.ex_fpstime]](0.05) );
		}
...
{
	if(!level.debugbotweapons) return;
